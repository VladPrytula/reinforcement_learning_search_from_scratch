...
- ❌ **Sample inefficiency**: Requires millions of episodes to converge
- ❌ **Exploration disasters**: Random exploration can destroy user experience
- ❌ **Black-box policies**: Neural networks are opaque to business stakeholders
- ❌ **Instability**: Training curves oscillate; hyperparameters are fragile
- ❌ **Cold-start failure**: No warm-start mechanism from domain knowledge
...
**Act I: Beautiful Theory (§6.1–6.3)**
...
§6.1 — **Discrete Template Action Space**: Define 8 interpretable boost strategies (High Margin, CM2 Boost, Premium, Budget, etc.)

§6.2 — **Thompson Sampling**: Bayesian posterior sampling with Gaussian conjugacy and ridge regression

§6.3 — **LinUCB**: Upper confidence bounds with confidence ellipsoids in feature space

§6.4 — **Production Code**: Full PyTorch implementation with type hints, batching, reproducibility
...
§6.8.4 — **Extensions & Practice**: Appendices covering neural extensions, theory-practice gaps, modern context, production checklists
...
- ✅ Thompson Sampling implementation with Bayesian ridge regression
- ✅ LinUCB implementation with confidence ellipsoids
- ✅ Production-quality code with full type hints and deterministic seeds
- ✅ Rich feature engineering pipeline (17 dimensions from user, query, products)
...
**Production skills:**
- ✅ Checking theorem assumptions before applying algorithms
- ✅ Diagnosing feature poverty vs. insufficient data vs. wrong algorithm
- ✅ Recognizing model misspecification in practice
- ✅ Fixing the real problem instead of trying random solutions

**Theoretical foundations:**
- ✅ Regret bounds for contextual bandits (conditional guarantees)
- ✅ Bayesian posterior sampling and Thompson Sampling convergence
- ✅ Upper confidence bounds and optimism under uncertainty
- ✅ Feature engineering as the performance ceiling
...
*Next: §6.1 — Discrete Template Action Space*
...
**Base relevance scale in our simulator**: From §5.2, lexical and embedding scores produce base relevance values $s_{\text{base}}(q,p) \in [10, 30]$ for relevant products, with typical standard deviation $\sigma \approx 5-8$ within a candidate set. Position bias and click propensities modulate this further (×0.3 to ×1.0).
...
- ✅ Fast learning (fewer arms to explore)
- ✅ Simple interpretation
- ❌ Limited expressiveness (may miss optimal strategies)
- ❌ Hard-codes too much prior knowledge

**Option B: Many templates (M=20-50)**
- ✅ Rich strategy space
- ❌ Slow learning (regret grows as $O(\sqrt{MT})$)
- ❌ Overfitting risk (too many options for limited data)
- ❌ Harder to debug

**Option C: Continuous parameterization**
- ✅ Maximum flexibility
- ❌ Loses interpretability (back to black-box)
...
def create_standard_templates(
    ...
    Implements the 8-template library from §6.1.1.
...
!!! note "Code ↔ Config (Template Library)"
    The template library from Table §6.1.1 maps to:
...
1. **Thompson Sampling (§6.2-6.3)**: Bayesian posterior sampling, probabilistic exploration
2. **LinUCB (§6.4-6.5)**: Frequentist upper confidence bounds, deterministic exploration
...
- ✅ No assumptions
- ❌ No generalization (each context learned independently)
- ❌ Infinite samples needed if $|\mathcal{X}|$ is large/continuous

**Option B: Nonlinear (neural network)**
- $\mu(x, a) = f_\theta(x, a)$ with neural net $f$
- ✅ Maximum flexibility
- ❌ Requires many samples (Chapter 7)
- ❌ Posterior intractable (no closed-form updates)

**Option C: Linear (our choice)**
- $\mu(x, a) = \theta_a^\top \phi(x)$
- ✅ Closed-form posterior (Gaussian conjugate)
- ✅ Sample-efficient with good features
- ❌ Misspecification risk (what if true $\mu$ is nonlinear?)
...
- **Optimized implementation:** Maintaining precision matrices and Cholesky factors across episodes reduces sampling to $O(M d^2)$ per step (rank‑1 updates + triangular solves; see §6.5 exercises).
...
**This is automatic!** No manually-tuned exploration rate (unlike ε-greedy) or confidence intervals (unlike UCB).
...
*Proof of Lemma 6.1.1.* This is a standard Gaussian tail bound applied to the one-dimensional projection $\tilde{\theta}_a^{(t)\top} \phi(x_t)$; see [@agrawal:thompson:2013, Lemma 3]. ∎
...
*Proof of Lemma 6.1.2.* See [@abbasi:improved:2011, Lemma 11]. ∎
...
5. **Final bound.**
   ...
   for a constant $C > 0$ depending only on $S, \sigma, \lambda_0, \delta$. ∎

**Remark 6.1.1** (When Regret Bounds Fail in Practice) {#REM-6.1.1}

[THM-6.1] is a **conditional guarantee**: regret is $O(\sqrt{T})$ *if the assumptions hold*. In §§6.5–6.6 we will deliberately violate these assumptions and see the regret picture break:

1. **Model misspecification (§6.6):** The true reward is nonlinear, but we force a linear model. The theorem still applies to the *best linear approximation*, yet approximation error dominates, and observed regret can grow almost linearly.
2. **Feature poverty (§6.6):** The feature map $\phi(x)$ omits critical context (e.g., user preferences). The bound applies in the restricted feature space, but the optimal policy **in that space** may be far from the true optimum.
...
The lesson is central to this chapter: *theorem correctness ≠ algorithm success*. In practice you must **monitor assumptions** via diagnostics (per-segment performance, uncertainty traces in §6.7) rather than trusting a regret bound in isolation.
...
# Output:
# Cumulative regret grows as O(√T) ✓
# Average regret per episode → 0 ✓
```
...
Theory works. In §6.4 we turn this into production code inside our simulator.
...
2. **UCB scores:** For each action, evaluating $\hat{\theta}_a^\top \phi_t$ is $O(d)$ and the naïve uncertainty term $\sqrt{\phi_t^\top A_a^{-1} \phi_t}$ requires forming $A_a^{-1}$, which is $O(d^3)$. Across all $M$ actions this is $O(M d^3)$.
...
Thus the **naïve total cost** is $O(T M d^3)$ over $T$ episodes; with rank‑1 updates and cached factorizations one obtains **$O(T M d^2)$**. Memory usage is $O(M d^2)$ for the design matrices and $O(M d)$ for the weight vectors. As in Thompson Sampling, our regime has small $M$ and moderate $d$, so these costs are negligible compared to simulator trajectories.
...
*Proof.* The Gaussian posterior mean under conjugate prior $\mathcal{N}(0, \lambda^{-1} I)$ and Gaussian likelihood is exactly the ridge regression minimizer. Both algorithms apply the same Bayesian update rule, differing only in action selection (sampling vs. UCB). ∎
...
### 6.3.2 Choosing the Exploration Parameter α
...
- ✅ Provably optimal regret
- ❌ Requires knowing $T$ (horizon) in advance
- ❌ Often overly conservative in practice

**Option B: Cross-validation**
- ✅ Data-driven tuning
- ❌ Expensive (requires offline simulation)
- ❌ May overfit to validation set

**Option C: Adaptive tuning** (our choice)
- ✅ Starts conservative, decays as confidence grows
...
Taking square root and union bound over $M$ actions yields [EQ-6.16]. ∎

**Comparison to Thompson Sampling:**

Both achieve $O(\sqrt{dT})$ regret (ignoring $\log$ factors). LinUCB has:
- ✅ **Deterministic**: Same trajectory with same data/seed
- ✅ **Interpretable**: UCB scores show why each action selected
- ❌ **Requires tuning**: Must choose $\alpha$ (TS is parameter-free)
- ❌ **Less adaptive**: Fixed exploration schedule (TS adapts naturally)
...
The previous sections treated Thompson Sampling and LinUCB as abstract algorithms. To run the experiments in §§6.5–6.7 we need **production-grade code** that:
...
2. A **policy class** exposes `select_action(φ(x))` and `update(a, φ(x), r)`.
...
- The config controls prior precision `λ`, noise scale `σ`, a `use_cholesky` flag, and a `seed` for reproducibility.
...
- On each call to `select_action(φ)` we:
...
- On each `update(a, φ, r)` we perform the Bayesian linear regression update from [EQ-6.8]:
  $$
...
This is exactly the mathematical algorithm from §6.2.2 written in NumPy/Torch, with care taken to keep matrices well-conditioned and sampling numerically robust.
...
!!! note "Code ↔ Algorithm (Thompson Sampling)"
    ...
    In the demos we always pass **feature vectors** `φ(x)` built by `context_features_simple` or `context_features_rich` from `scripts/ch06/template_bandits_demo.py`, so the production code is agnostic to how features are constructed.
...
class LinUCBConfig:
    ...
    Attributes:
        ...
        adaptive_alpha: If True, use α_t = α√log(1 + t)
...
class LinUCB:
    ...
    action selection. Both maintain identical posterior means θ̂_a but differ
    ...
    def select_action(self, features: NDArray[np.float64]) -> int:
        ...
        for a in range(self.M):
            ...
            # Uncertainty bonus: α √(φ^T A_a^{-1} φ), implementing [EQ-6.14]
    ...
    def update(
        ...
        # Update reward accumulator: b_a ← b_a + r φ
        self.b[a] += reward * phi

        # Update weight estimate: θ̂_a ← A_a^{-1} b_a (via solve)
...
!!! note "Pedagogical Design: Feature Engineering as Iterative Process"
    ...
    **Why?** Feature engineering is iterative: start simple (segment + query type), evaluate, diagnose failures, then add richer signals. Starting with 7 dimensions is:
    ...
    - ✅ Faster to train (lower-dimensional posteriors)
    - ✅ Avoids premature overfitting (letting the model "see" everything)
...
3. **Thompson Sampling.** Train for 20 000 episodes with $\phi_{\text{simple}}$, same regularization and noise scale as in §6.4.
...
On one side you have the theorems from §6.2 and §6.3 telling you:
...
In §6.6 we will resist the temptation to blame bugs or hyperparameters and instead put the theorems themselves on the table. We will read the fine print and see exactly which assumption we violated.
...
We can rule out (1): the posterior updates match the closed‑form equations in §6.2 and §6.3, tests in `tests/ch06/` pass, and 20 000 episodes is plenty for an 8‑arm bandit. We can rule out (2): [THM‑6.2] is a widely used result (Abbasi‑Yadkori et al.), with a proof that has been checked many times over.
...
- Prices cluster around €15 with healthy margins.
...
- Prices cluster around €40 with thinner margins.
...
**Caption:** Episodes A and B are indistinguishable to the bandit (identical $\phi_{\text{simple}}$) but yield **4× different GMV**. The missing information is the PL fraction in the base ranker's top-K—a critical context the simple features don't capture.
...
The take‑away from §6.6 is not “LinUCB is bad” or “Thompson Sampling fails.” It is:
...
In §6.7 we fix the right thing: we redesign the features.
...
We now run exactly the same experiment as in §6.5, but with $\phi_{\text{rich}}$ containing the **true user latent preferences**:
...
- **Discrete template action space** (§6.1): 8 interpretable boost strategies encoding business logic (High Margin, CM2 Boost, Premium, Budget, Discount, etc.).
- **Thompson Sampling and LinUCB theory** (§§6.2–6.3): posterior sampling and UCB for linear contextual bandits, with $O(\sqrt{T})$‑type regret guarantees.
- **Production‑quality implementations** (§6.4): type‑hinted NumPy/Torch code in `zoosim/policies/{thompson_sampling,lin_ucb}.py`, wired to the simulator via `scripts/ch06/template_bandits_demo.py`.
...
If you plan to run many seeds, feature variants, or large episode counts (100k+), the CPU implementation (§6.4) becomes slow (~30 seconds per run). See the optional **Advanced Lab 6.A: From CPU Loops to GPU Batches** (`ch06_advanced_gpu_lab.md`).
...
- **Exercise 6.2**: Implement ε-greedy baseline, compare regret to LinUCB
...
- ✅ Bandits beat best static template by **+1-7% GMV** (context-dependent improvement)
- ✅ Regret grows sublinearly: $O(\sqrt{T})$ empirically (theory confirmed)
- ✅ Exploration decays automatically (no manual schedule needed)
- ✅ Interpretable policies (stakeholders see which template selected, why)
...
Created files:
- ✅ `zoosim/policies/templates.py` (template library + application logic)
- ✅ `zoosim/policies/thompson_sampling.py` (Bayesian posterior sampling)
- ✅ `zoosim/policies/lin_ucb.py` (UCB for linear contextual bandits)

Updated files:
- ✅ `zoosim/policies/__init__.py` (export template bandits)

Test files:
- ✅ `tests/ch06/test_templates.py` (template application, boundedness)
- ✅ `tests/ch06/test_thompson_sampling.py` (posterior updates, convergence, diagnostics)
- ✅ `tests/ch06/test_linucb.py` (UCB scores, ridge regression equivalence, TS mean match)
- ✅ `tests/ch06/test_integration.py` (full training loops with mock environment)

Documentation:
- ✅ `docs/book/drafts/ch06/discrete_template_bandits.md` (this chapter)
- ✅ `docs/book/drafts/ch06/exercises_labs.md` (exercises and labs)

Knowledge Graph entries:
- ✅ Chapter and theory nodes for Ch06 (`CH-6`, `DEF-6.1`–`DEF-6.4`, `ALG-6.1`, `ALG-6.2`, `THM-6.1`, `THM-6.2`, `PROP-6.1`, `PROP-6.2`, `EQ-6.1`–`EQ-6.17`)
- ✅ Module nodes for `zoosim/policies/templates.py`, `zoosim/policies/thompson_sampling.py`, `zoosim/policies/lin_ucb.py`
- ✅ Test nodes for `tests/ch06/test_templates.py`, `tests/ch06/test_thompson_sampling.py`, `tests/ch06/test_linucb.py`, `tests/ch06/test_integration.py`
