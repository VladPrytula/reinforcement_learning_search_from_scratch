...
$$
that assigns a bounded boost value to each product $p \in \mathcal{C}$. Given a query result set $\{p_1, \ldots, p_K\} \subset \mathcal{C}$ and base relevance scores $s_{\text{base}}(q, p_i)$ from [DEF-5.1] (`docs/book/ch05/ch05_relevance_features_reward.md:86`), the template induces **adjusted scores**:
$$
...
This aligns with the Bandit Bellman operator [DEF-3.8.1] (`docs/book/ch03/ch03_stochastic_processes_bellman_foundations.md:748`), where setting $\gamma = 0$ eliminates temporal credit assignment. Templates become arms in a contextual bandit; the learning problem is to identify which arm maximizes immediate reward given the context.
...
!!! note "Units and scales (simulator defaults)"
    - Base relevance $s_{\text{base}}(q,p)$ is a dimensionless score (Chapter 5). In Lab 2.2 we observe mean $\approx 0.10$, std $\approx 0.22$, min $\approx -0.56$, max $\approx 0.93$ over 100 queries × 100 products; in Chapter 5 top-$K$ examples reach roughly $1.2$. These summary statistics are computed by `scripts/ch06/template_bandits_demo.py` (default seeds); see the batch outputs under `docs/book/ch06/data/`.
...
Throughout Section 6 we work with the disjoint-parameter model (one unknown vector per template), which yields a $\sqrt{M}$ dependence in worst-case regret when combining per-template bounds.
...
!!! note "Shared vs. disjoint linear bandits"
    Many references analyze the **shared-parameter** model $\mu(x,a)=\langle \theta^*, \phi(x,a)\rangle$, where a single unknown vector $\theta^*$ governs all actions via action-dependent features $\phi(x,a)$. In that setting, minimax regret scales as $\tilde{O}(d\sqrt{T})$ with $d=\dim(\phi)$ (no explicit $\sqrt{M}$ factor).

    In this chapter we use the **disjoint-parameter** model $\mu(x,a)=\langle \theta_a^*, \phi(x)\rangle$, with one parameter vector per template. This aligns with the implementation (we maintain separate ridge regressions/posteriors per template) and yields worst-case bounds with a $\sqrt{M}$ dependence (we effectively learn $M$ separate linear models).
...
Both notions can be analyzed under either the shared-parameter model $\mu(x,a)=\langle \theta^*, \phi(x,a)\rangle$ or the disjoint-parameter model $\mu(x,a)=\langle \theta_a^*, \phi(x)\rangle$. The $\sqrt{M}$ dependence in our worst-case bounds comes from the disjoint-per-template parameterization (see the note after [DEF-6.3]), not from choosing Bayesian vs. frequentist regret.
...
3. **Bounded features**: $\|\phi(x)\| \leq L$ for all $x \in \mathcal{X}$ and some constant $L > 0$
4. **i.i.d. contexts**: Contexts $\{x_t\}_{t=1}^T$ are drawn i.i.d. from distribution $\rho$ over $\mathcal{X}$. In our simulator, each episode samples a fresh user and query independently (see `scripts/ch06/template_bandits_demo.py:522-523`), and then simulates the session given that context (see `scripts/ch06/template_bandits_demo.py:442` and `scripts/ch06/template_bandits_demo.py:479-486`).
...
If the noise is truly Gaussian, the updates in [ALG-6.1] correspond to the exact conjugate posterior. Under merely sub-Gaussian noise, we use $\mathcal{N}(\hat\theta_a,\Sigma_a)$ as a Gaussian exploration distribution; the regret analysis relies on self-normalized concentration of the ridge estimator and Gaussian sampling tails, rather than on exact Bayesian correctness.

Caveat: GMV may exhibit heavy tails due to (approximately) lognormal prices (Appendix 6.B). In that case, the sub-Gaussian noise assumption can fail, and fully rigorous bounds require clipping/robust regression variants; see [REM-6.1.1] and Appendix 6.B.
...
Constant $C > 0$ depends only on $(S, L, \sigma, \lambda_0)$ and is independent of $T$.
...
*Proof (reference).* A complete analysis for Thompson Sampling in the **shared-parameter** linear contextual bandit appears in [@agrawal:thompson:2013, Theorem 2]. The disjoint-per-action form stated here follows by applying the same self-normalized ridge-regression concentration tools separately to each action and combining the resulting bounds across actions (yielding the $\sqrt{M}$ dependence via Cauchy--Schwarz); see also [@lattimore:bandit_algorithms:2020, Chapters 19 and 35]. $\square$
...
$\|v\|_{\Sigma} := \sqrt{v^\top \Sigma v}$ (all vector norms are Euclidean; matrix-weighted norms use the positive-definite metric as stated).
...
Let $(\mathcal{X}, \mathcal{A}, R)$ be a linear contextual bandit with the same assumptions as [THM-6.1]. Fix a regularization parameter $\lambda > 0$ and a confidence parameter $\delta \in (0,1)$. Then LinUCB ([ALG-6.2]) with exploration parameter
$$
\alpha
...
- (A2) Bounded features holds with $L = 1$ here: $\phi_{\text{simple}}$ is one-hot, so $\|\phi\|_2 = 1$.
