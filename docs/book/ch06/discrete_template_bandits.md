...
For any learning algorithm and any time horizon $T \geq M$, there exists an $M$-armed stochastic bandit instance such that the expected cumulative regret satisfies:
...
*Proof.* Appendix D states this lower bound for $K$ arms (Theorem D.3.1). Substituting $K = M$ yields the claim. $\square$
...
**Lemma 6.1.1** (Gaussian Concentration for Posterior Samples) {#LEM-6.1.1}
...
4. **Uncertainty accumulation.**
   The posterior covariance $\Sigma_{a_t}$ evolves as a rank-one update driven by $\phi_t$. In the disjoint (per-action) model, applying Lemma 6.1.2 separately to each action and summing yields
   $$
   \sum_{t=1}^T \|\phi_t\|_{\Sigma_{a_t}}^2
     \leq 2d \sum_{a=1}^M \log\bigl(1 + n_a(T)/(d \lambda_0)\bigr)
     \leq 2dM \log\bigl(1 + T/(d \lambda_0)\bigr),
   $$
   where $n_a(T)$ is the number of times action $a$ is selected up to time $T$.
