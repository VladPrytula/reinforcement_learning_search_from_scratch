...
\hypersetup{
    ...
    pdftitle={Chapter 1: Search Ranking as Optimization},
...
\title{Chapter 1: Search Ranking as Optimization}
...
\item
  \textbf{Strategic goals (STRAT)}: Expose new products, house brands,
  or inventory to clear
\item
...
\textbf{Spaces (Working Definitions).} \label{DEF-1.1.0}
...
In Chapter 0 (Motivation: Your First RL Experiment), we built a tiny,
...
cross-reference. Anchors like \texttt{{[}{]}\{\#THM-1.7.2\}} enable
internal linking.
\end{quote}
...
In the reward {[}EQ-1.2{]}, \(\text{STRAT}(\omega)\) counts
\textbf{strategic purchases} in the session (purchased items whose
\texttt{strategic\_flag} is true). In guardrails like {[}EQ-1.3b{]}, we
instead track \textbf{strategic exposure}---how many strategic items
...
This is the set-based stability metric used in Chapter 10 \ref{DEF-10.4}
and implemented in \texttt{zoosim/monitoring/metrics.py:89-118}. A
...
implement \eqref{EQ-1.3} constraint definitions:

\begin{itemize}
\tightlist
\item
  Rank stability penalty weight: \texttt{lambda\_rank} in
  \texttt{zoosim/core/config.py:230}
\item
  Profitability floor (CM2): \texttt{cm2\_floor} in
  \texttt{zoosim/core/config.py:232} (defined in config; enforcement is
  introduced in Chapter 10)
\item
  Exposure floors (strategic products): \texttt{exposure\_floors} in
  \texttt{zoosim/core/config.py:233} (defined in config; enforcement is
  introduced in Chapter 10)
\end{itemize}

These are surfaced in \texttt{ActionConfig} and wired into downstream
modules as they mature.

\end{CalloutNote}

\subsubsection{\texorpdfstring{1.2.1 The Role of Engagement in Reward
Design
\label{REM-1.2.1}}{1.2.1 The Role of Engagement in Reward Design }}\label{the-role-of-engagement-in-reward-design}
...
\item
  ...
  revenue)
\item
  \(\gamma/\alpha\) (STRAT weight): Strategic priority, typically
  \(\in [0.1, 0.3]\) (house brands, new products, clearance)
\item
...
\subsubsection{\texorpdfstring{Problem Setup
\label{DEF-1.4.1}}{Problem Setup }}\label{problem-setup}

\textbf{Working Definition 1.4.1} (Contextual Bandit for Search
Ranking). \label{WDEF-1.4.1}
...
\item
  ...
  \texttt{zoosim/envs/search\_env.py:50}
\item
...
\CommentTok{\# Zero action of correct dimensionality; env will clip if needed (see \textasciigrave{}zoosim/envs/search\_env.py:50\textasciigrave{}).}
...
\CommentTok{\# Sample a valid bounded action (env clips incoming actions internally as well)}
...
Constraint enforcement for CM2 floors and rank stability appears in
Chapter 10 (Guardrails), with the underlying duality theory in Appendix
C.
...
detection, rank stability, CM2 floors, A/B testing, monitoring)
\textbf{Chapter 11}: Multi-episode MDPs (inter-session retention, hazard
modeling, long-term user value)
...
\begin{quote}
\textbf{OPE ≈ ``reweight logged rewards by importance weights.''}
\end{quote}
...
We enforce constraints via \textbf{Lagrangian methods} (Chapter 10, with
theory in Appendix C) and \textbf{rank stability penalties}.
...
\textbf{Theorem 1.9.1 (Slater's Condition, informal).} \label{THM-1.9.1}
If there exists at least one policy that strictly satisfies all
constraints (e.g., a policy with CM2 and exposure above the required
floors and acceptable rank stability), then the \textbf{Lagrangian
...
({[}@boyd:convex\_optimization:2004, Section 5.2.3{]}). Chapter 10
exploits this to implement \textbf{primal--dual RL} for search: we
...
violations (dual step).

\textbf{What this tells us:}

Strong duality means we can solve the constrained problem
\eqref{EQ-1.21}1 by solving the unconstrained Lagrangian \#EQ-1.22---no
duality gap. Practically, this justifies \textbf{primal-dual
...
\textbf{Implementation preview}: In \textbf{Chapter 10 (§10.4.2)}, we
implement constraint-aware RL using primal-dual optimization (theory in
\textbf{Appendix C, §C.5}): 1. \textbf{Primal step}:
...
\item
  \textbf{Align action bounds}: \texttt{SimulatorConfig.action.a\_max}
  in \texttt{zoosim/core/config.py:229}; examples should respect the
  same value.
\item
  \textbf{Use config-driven weights}: \texttt{RewardConfig} for
...
\item
  ...
  drops \(>10\%\) (clickbait detection).
\item
  \textbf{Enforce constraints early}: CM2 and exposure floors via
  Lagrange multipliers (\textbf{Chapter 10, §10.4.2}; theory in
  \textbf{Appendix C}).
\item
  \textbf{Ensure reproducible ranking}: Enable
